...
export const CartModal: React.FC<CartModalProps> = ({ cart, onClose, onRemove, onClear }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const mailto = Boolean(import.meta.env.VITE_ORDER_EMAIL?.trim());
      alert(
        mailto
          ? 'Текст заказа скопирован в буфер обмена. Если открылся почтовый клиент — отправьте письмо. Иначе вставьте текст из буфера в мессенджер.'
          : 'Текст заказа скопирован в буфер обмена. Вставьте его в WhatsApp, Telegram или другое приложение и отправьте нам.'
      );
      onClear();
      onClose();
    } catch (error) {
