...
// Отдельный компонент для картинки с состоянием загрузки
...
export const Catalog: React.FC<CatalogProps> = ({ tomatoes, cartItems, onAddToCart, onViewDetail }) => {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
      {tomatoes.map((tomato) => {
        const isInCart = cartItems.some(item => item.tomato.id === tomato.id);
        ...
        if (tomato.color === "Yellow") badgeColor = "bg-yellow-100 text-yellow-800";
        if (tomato.color === "Orange") badgeColor = "bg-orange-100 text-orange-800";
        if (tomato.color.includes("Black")) badgeColor = "bg-slate-800 text-white";
        ...
        return (
          ...
          >
            <div className="relative h-48 overflow-hidden bg-stone-100 cursor-pointer" onClick={() => onViewDetail(tomato)}>
            ...
            <div className="p-4 flex-grow flex flex-col">
              <div className="flex justify-between items-start mb-2">
                 <div>
                    <h3 className="font-bold text-stone-800 text-lg leading-tight group-hover:text-tomato-600 transition-colors cursor-pointer" onClick={() => onViewDetail(tomato)}>
                      {tomato.name}
                    </h3>
                    <div className="flex gap-2 mt-2">
                       <span className={`text-[10px] uppercase tracking-wider font-semibold px-1.5 py-0.5 rounded ${badgeColor}`}>
              ...
              <div className="mt-auto pt-4">
                ...
                >
                  {isInCart ? <><Check size={16}/> Добавлено</> : <><Plus size={16} /> В список</>}
                </button>
              </div>
            </div>
          </div>
